...
export const signUp = async (req, res) => {
  try {
    ...
    // mã hóa password
    const hashedPassword = await bcrypt.hash(password, 10); // hàm bcrypt sẽ tạo ra một chuỗi ngẫu nhiên và chèn vào pwd và salt = 10 là mã hóa 2^10 lần
...
export const signIn = async (req, res) => {
  try {
    ...
    // tạo refresh token
    const refreshToken = crypto.randomBytes(64).toString("hex"); // tạo ra 64 bytes ngẫu nhiên và chuyển sang chuỗi hex
    ...
    res.cookie("refreshToken", refreshToken, {
      httpOnly: true, // không cho truy cập bằng js
      secure: true, // để đảm bảo chỉ gửi qua https
